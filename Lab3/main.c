...
#define CONFIG_EXAMPLE_FIRMWARE_UPGRADE_URL "https://192.168.89.22:5000/firmware.bin" 

#define CONFIG_EXAMPLE_VERSION_URL "https://192.168.89.22:5000/version"
...
char* get_server_version() {
    esp_http_client_config_t config = {
        .url = CONFIG_EXAMPLE_VERSION_URL,
...
static void ota_task(void *pvParameters)
{
    ...
    fptr = fopen("versioning", "r");
    // Store the content of the file
    char current_version[100];
    // Read the content and store it inside myString
    fgets(current_version, 100, fptr);
    // Close the file
    fclose(fptr);*/
    char* current_version = "1";

    char* server_version = get_server_version();
    if (server_version == NULL) {
        ESP_LOGE(TAG, "Failed to get server version");
        vTaskDelete(NULL);
    }
